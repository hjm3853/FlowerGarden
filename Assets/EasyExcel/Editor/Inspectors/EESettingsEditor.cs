...
namespace EasyExcel
{
	public class EESettingsEditor : EditorWindow
	{
		...
		private void OnGUI()
		{
			...
			GUILayout.Space(5);
			if (settings.ShowHelp)
				GUILayout.Label(GetSettingFieldComment("dbKind"), EEGUIStyle.helpBox, GUILayout.Width(helpLength));
			GUILayout.BeginHorizontal();
			GUILayout.Label(settings.Lang == EELang.CN ? "dbKind" : "Row of Data", EEGUIStyle.label, nameOptions);
			settings.DBKind = EditorGUILayout.IntField(settings.DBKind, EEGUIStyle.textField, valueOptions);
			GUILayout.EndHorizontal();
			if (settings.ShowHelp)
				GUILayout.Space(tipSpace);
			...
			GUILayout.Space(5);
			if (settings.ShowHelp)
				GUILayout.Label(GetSettingFieldComment("generatedClientAssetPath"), EEGUIStyle.helpBox, GUILayout.Width(helpLength));
			GUILayout.BeginHorizontal();
			GUILayout.Label(settings.Lang == EELang.CN ? "生成资源文件路径" : "ClientAssetPath", EEGUIStyle.label, nameOptions);
			settings.GeneratedClientAssetPath = EditorGUILayout.TextField(settings.GeneratedClientAssetPath, EEGUIStyle.textField, valueOptions);
			GUILayout.EndHorizontal();
			if (settings.ShowHelp)
				GUILayout.Space(tipSpace);
			...
			GUILayout.Space(5);
			if (settings.ShowHelp)
				GUILayout.Label(GetSettingFieldComment("generatedServerAssetPath"), EEGUIStyle.helpBox, GUILayout.Width(helpLength));
			GUILayout.BeginHorizontal();
			GUILayout.Label(settings.Lang == EELang.CN ? "生成资源文件路径" : "ServerAssetPath", EEGUIStyle.label, nameOptions);
			settings.GeneratedServerAssetPath = EditorGUILayout.TextField(settings.GeneratedServerAssetPath, EEGUIStyle.textField, valueOptions);
			GUILayout.EndHorizontal();
			if (settings.ShowHelp)
				GUILayout.Space(tipSpace);
