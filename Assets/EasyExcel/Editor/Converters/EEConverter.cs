...
namespace EasyExcel
{
	...
	public static partial class EEConverter
	{
		...
		private static SheetData ToSheetDataRemoveEmptyColumn(EEWorksheet sheet)
		{
			...
			{
				var cellValue = sheet.GetCellValue(OdinEasyExcelMenuTree.kTotalMenu.GetSetting().kDataType, column);
			...
			{
				var cellValue = sheet.GetCellValue(OdinEasyExcelMenuTree.kTotalMenu.GetSetting().kPropertyType, validNameColumns[column]);
		...
		{
			if (sheet == null || sheet.RowCount <= OdinEasyExcelMenuTree.kTotalMenu.GetSetting().kPropertyType || sheet.ColumnCount < 1)
				return false;
			int validColumnCount = 0;
			for (int col = 0; col < sheet.ColumnCount; col++)
			{
				string varType = sheet.GetCellValue(OdinEasyExcelMenuTree.kTotalMenu.GetSetting().kPropertyType, col);
				if (string.IsNullOrEmpty(varType) || varType.Equals(" ")  || varType.Equals("\r"))
					continue;
				if (EEColumnFieldParser.IsSupportedType(varType))
				{
					string varName = sheet.GetCellValue(OdinEasyExcelMenuTree.kTotalMenu.GetSetting().kDataType, col);
